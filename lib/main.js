...
class ProjectList {
  constructor(state) {
    ...
    this.selectList = lumine.workspace.buildSelectList({
      ...
      confirmAction: "project-list:open-in-new-window",
      additionalActionCommands: ["project-list:edit", "project-list:clear-recent"],
      ...
      idForItem: (item) => this.projectKey(item),
      ...
      actionsFilter: ({ name }) => {
        if (name === "project-list:clear-recent") return this.recentlyUsed.length > 0;
        return (
          name !== "project-list:remove-from-recent" ||
    ...
    this.disposables.add(
      ...
      // on `lumine-workspace` too would be filtered out of the actions list as
      // chrome from above.
      lumine.commands.add(this.selectList.element, {
        ...
        "project-list:remove-from-recent": {
          description: "Drop the selected project from the recent section, leaving the list open.",
  ...
  isRecent(item) {
    const key = item == null ? null : this.projectKey(item);
    return Boolean(key) && this.recentlyUsed.includes(key);
  }

  // Forgetting one project is something a user does to several in a row, so
  // the list stays open and the row keeps its selection — it only moves out
  // of the section, down to where its own title puts it.
  removeFromRecent() {
    ...
    this.refreshRecent();
    // A project is rebuilt on every scan, so the row to keep selected is
    // found again by key rather than by the object the action started from.
    const row = this.selectList.items?.find((each) => this.projectKey(each) === key);
  ...
  // The list only re-reads `recentIds` when it is handed them.
  refreshRecent() {
  ...
  async updateView(loadCache = true) {
    ...
    this.selectList.update({
      ...
      status: errors.length ? { type: "error", message: errors.join("\n"), sticky: true } : null,
    });
