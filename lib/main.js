...
class ProjectList {
  constructor() {
    ...
    this.disposables.add(
      lumine.applicationDelegate.onDidReceiveWindowEvent(
  ...
  performAction(mode) {
    ...
    if (mode === "open-in-new-window") {
      lumine.app.openWindow({ ...data, newWindow: true });
    } else if (mode === "open-in-dev-mode") {
      lumine.app.openWindow({ ...data, newWindow: true, devMode: true });
    } else if (mode === "open-in-safe-mode") {
      lumine.app.openWindow({ ...data, newWindow: true, safeMode: true });
      ...
      if (!editor) {
        lumine.notifications.addError("Cannot insert path, because there is no active text editor");
        return;
      }
